...
def plot_bar(top_words):
    ...
    c.add_yaxis("频率", list(frequencies))
    c.set_global_opts(title_opts=opts.TitleOpts(title="词频条形图"))
...
def plot_pie(top_words):
    ...
    c.set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {c}", rich={"color": ["#334455", "#2a333d", "#87CEFA"],
                                                                            ...
                                                                            "shadowBlur": 10}))
    c.set_global_opts(
        title_opts=opts.TitleOpts(title="词频饼状图", pos_left="center",
...
def plot_line(top_words):
    ...
    c = Line()
    c.add_xaxis(words)
    c.add_yaxis("频率", frequencies)
    c.set_global_opts(title_opts=opts.TitleOpts(title="词频线状图"))
...
def plot_scatter(top_words):
    ...
    c.add_xaxis(words)
    c.add_yaxis("频率", frequencies)
    c.set_global_opts(title_opts=opts.TitleOpts(title="词频散点图"))
...
def plot_funnel(top_words):
    ...
    c.set_global_opts(title_opts=opts.TitleOpts(title="词频漏斗图"))
...
def plot_polar(top_words):
    ...
    c = Polar()
    c.add("", list(zip(words, frequencies)))
    c.set_global_opts(title_opts=opts.TitleOpts(title="词频极坐标图"))
...
def plot_radar(top_words):
    ...
    c.add("", [frequencies])
    c.set_global_opts(title_opts=opts.TitleOpts(title="词频雷达图"))
